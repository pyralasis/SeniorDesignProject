<script lang="ts">
  ...
  import TrainingTable, { type ColumnDef, type CompleteTrainingStatus, type FailedTrainingStatus, type InProgressTrainingStatus, type LoadedTrainingObj, type QueuedTrainingStatus, type TrainingTaskContent, type TrainingTaskInfo } from '$lib/components/Training/TrainingTable.svelte';
  ...
  $effect(() => {
    const interval = setInterval(async () => {
        ...
        availableTasks = await Promise.all(availableTrainingTasks.map(async (info) => await BackendApi.getTrainingTaskByID(info.id)))
    }, 1000);
  ...
  let onSelectTask = (value: number) => {selectedTask = availableTasks.filter((task) => task.id == value)[0].content};
</script>
...
<div class="training-page">
        ...
        <div style="height: 38px; display: flex; margin: auto 0;">
            <Button
                ...
                on:click={() => {
                    ...
                    availableTasks = availableTasks.filter(tsk => tsk.content.data.status !== "complete" && tsk.content.data.status !== "failed")
                }}
            >Delete All</Button>
        </div>
  ...
  <div class="training-page__top">
        <TrainingTable name="In Progress" status="in_progress" columns={in_progress_columns} available_tasks={availableTasks} on_select={onSelectTask}></TrainingTable>
        <TrainingTable name="Queued" status="queued" columns={queued_columns} available_tasks={availableTasks} on_select={onSelectTask}></TrainingTable>
        <TrainingTable name="Complete" status="complete" columns={completed_columns} available_tasks={availableTasks} on_select={onSelectTask} can_delete on_delete={delete_train_task}></TrainingTable>
        <TrainingTable name="Failed" status="failed" columns={failed_columns} available_tasks={availableTasks} on_select={onSelectTask} can_delete on_delete={delete_train_task}></TrainingTable>
  </div>
